  ...
  for (let cronjob of cronjobs) {
    ...
    for (let job of valuesFileJson.jobs) {
      if (cronjobName.endsWith(`-${job.name}`) && job.enabled) {
        if ("suspend" in job) {
          const patchStr = JSON.stringify({ spec: { suspend: job.suspend } });
          promises2.push(kubectlPatch(["cronjob", cronjobName, "--patch", patchStr]));
        } else {
          core2.warning(`scaling addons ${job.name}, missing suspend in values file, not setting it`);
        }
      }
    }
  }
