...
var require_compose_scalar = __commonJS({
  "node_modules/yaml/dist/compose/compose-scalar.js"(exports) {
    ...
    function findScalarTagByTest(
      ...
      const tag =
        schema.tags.find((tag2) => tag2.default && tag2.test?.test(value)) ||
...
// src/change-ingress-class/main.ts
...
async function kubectlPatch(args) {
  ...
  return exec.exec(
    "kubectl",
    ["--namespace", ci.namespace, "patch"].concat(args),
    opts.options
  );
}

// src/change-ingress-class/index.ts
var core2 = __toESM(require_core());
async function changeIngressClass() {
  const mode = core2.getInput("mode", { required: true });
  const classInput = core2.getInput("class", { required: true });
  const ingresses = await kubectlGet(["ingress"]);
  if (ingresses.length === 0) {
    throw new Error(
      "Failed - changeIngressClass, no deploys matched with regexp"
    );
  }
  ...
  for (let ingress of ingresses) {
    ...
    const originalClass = ingress.spec.ingressClassName;
    let newClassName = originalClass;
    switch (mode) {
      case "append":
      ...
      default:
        throw new Error(
          'Failed - changeIngressClass, "mode" not set correctly'
        );
    }
    const patchStr = JSON.stringify({
      spec: { ingressClassName: newClassName },
    });
