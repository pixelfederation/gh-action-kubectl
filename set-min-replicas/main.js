...
// src/set-min-replicas/main.ts
var core3 = __toESM(require_core());

// src/set-min-replicas/index.ts
...
// src/set-min-replicas/index.ts
async function setMinReplicas() {
  const replicas = core2.getInput("replicas", { required: true });
  ...
  for (let hpa of hpas) {
    ...
    const patchStr = JSON.stringify({
      spec: { minReplicas: parseInt(replicas) },
    });
